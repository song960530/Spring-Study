...
- Server: 요청을 처리하는 오리진서버의 소프트웨어 정보(실제로 중간중간 프록시 서버나 캐시서버를 거쳐서 실제 서버로 도착한다)
- Date: 메세지가 생성된 날짜
- Host: 요청한 호스트 정보(도메인). Host정보는 필수이다
- Location: 페이지 리다이렉션
- Allow: 허용 가능한 ATTP 메서드
- Retry-After: 유저 에이전트가 다음 요청을 하기까지 기다려야하는 시간(거의 사용하지 않는다)
...
- 쿠키를 사용할때의 간략한 매커니즘은 아래와 같이 동작한다
![쿠키 간단 메커니즘1](https://user-images.githubusercontent.com/52727315/166423122-19396ab4-ee12-4079-9297-52914f1dfc10.png)
![쿠키 간단 메커니즘2](https://user-images.githubusercontent.com/52727315/166423125-b17fca06-f417-4b1a-8ea3-34e71cfb610c.png)
- 쿠키를 사용할 땐 최소한의 정모만 사용하도록 한다(세션 ID, 인증 토큰 등)
- 서버에 전송하지 않고 웹 브라우저 내부에 저장하고 싶을 땐 웹 스토리지를 사용한다
- 보안에 민감함 데이터는 저장하면 안된다
- 생명주기
  - Set-Cookie: expires=Sat, 26-Dec-2020 04:39:21 GMT -> 만료일이 되면 쿠키 삭제
  - Set-Cookie: max-age=3600 (3600초) -> 특정 시간 이후에 쿠키 삭제
  - 세션 쿠키: 만료날짜를 생략하면 브라우저 종료시까지만 유지
  - 영속 쿠키: 만료날짜를 입력하면 해당 날짜까지 유지
- 도메인
  - 명시: 명시한 문서 기준 도메인 + 서브 도메인 포함
    - EX) domain=example.org로 설정하면 dev.example.org에서도 접근이 가능하다
  - 생략: 현재 문서 기준 도메인만 적용
    - EX) 도메인 설정을 생략하면 생성한 도메인에서만 접근이 가능하자
- 경로(Path)
  - path=/example로 설명하였을경우 /example 포함 하위 경로에서까지만 접근이 가능하다
- 보안
  - Secure: 쿠키는 http, https 구분하지않고 전송하는데 해당옵션을 적용하면 https인 경우에만 전송한다
  - HttpOnly: Xss공격 방지, 자바스크립트에서 접근 불가, HTTP전송에만 사용
  - SameSite: XSRF공격 방지, 요청 도메인과 쿠키에 설정된 도메인이 같을 경우만 쿠키 전송
