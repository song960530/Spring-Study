...
```
-  
  - 설정 정보에 초기화, 소멸 메서드 이름을 지정해서 사용할 수 있다
  - 메서드 이름을 자유롭게 줄 수 있다
  - 스프링 빈이 스프링 코드에 의존하지 않는다
  - 코드가 아니라 설정 정보를 사용하기 때문에 코드를 고칠 수 없는 외부 라이브러리에도 초기화, 종료 메서드를 적용할 수 있다
  - 추가적으로 destroyMethod 설정은 (inferred) 라고 설정되어 있으며, 별도의 설정을 하지 않을 경우 close나 shutdown 메서드를 자동으로 호출해주도록 되어있다.
- ✨@PostConstruct, @PreDestroy 어노테이션
  - 최신 스프링에서 가장 권장하는 방법
  - 패키지를 보면 ```javax.annotation.PostConstruct``` 으로 스프링에 종속적인 기술이 아닌 자바 표준 기술이다
  - 유일한 단점은 외부 라이브러리에는 적용하지 못한다는 것이다. 외부 라이브러리를 초기화, 종료 해야 한다면 설정 정보에 지정해주는 방법을 사용하자
...
## 빈 스코프🧐
- 빈이 존재할 수 있는 범위를 뜻한다
- 크게 싱글톤, 프로토타입, 웹관련 스코프로 나눠진다
- 싱글톤 : 기본 스코프, 스프링 컨테이너의 시작과 종료까지 유지되는 가장 넓은 범위의 스코프
- 프로토타입 : 빈의 생성과 의존관계 주입까지만 관여하고 더는 관리하지 않는
매우 짧은 범위의 스코프이다
- 웹 관련 스코프
...
```
- 위와 같이 proxyMode 설정을 추가하게 되면 스프링 빈 등록 시 가짜 프록시 클래스를 만들어 미리 등록해둔다
- 해당 기술도 CGLIB라는 바이트코드 조작 라이브러리를 사용하여 MyLogger를 상속받은 가짜 객체를 생성하는 것 이다
- 이후 가짜 프록시 객체는 요청이 들어오면 내부에서 진짜 빈을 요청하는 위임 로직을 실행한다
  - 클라이언트가 호출하는 myLogger는 사실 가짜 프록시이고 가짜 프록시 객체가 진짜 객체를 호출하는 것 이다


## Scope를 정리하면서..📝
- 싱글톤 스코프 빈을 제외한 다른 타입으 빈들을 사용할 땐 결국 주위해서 사용해야 한다
- 이런 특별한 scope는 꼭 필요한 곳 에서만 최소화하여 사용하며, 무분별하게 사용하면 유지보수하기 어려워진다
